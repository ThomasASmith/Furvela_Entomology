...
# create the colour palette. This is a re-ordering of brewer.pal(n = 5, name = "Paired")
get_palette <- function() {
  return(c("#b2df8a", "#1f78b4", "#a6cee3", "#33a02c", "#FB9A99"))
}
...
get_Furvela_data <- function() {
  ...
  data_description <- function(datalist) {
    ...
    drops <- c("Casa", "Data", "data", "Sheet", "ISO_Week", "Tempo_aqui", "Nome_de_fora", "Cumprimento", "Largura",
      "Preco", "period_lado", "Name", "Observacoes", "Age", "preco..mil.", "PIN", "Ano_Nasc", "Data_Nasc", "number",
      "plate")
    ...
    collection_table <- mapply(table, datalist$mosquito_collection[, !(names(datalist$mosquito_collection) %in%
      drops)])
    collection_summary <- mapply(summary, datalist$mosquito_collection[, !(names(datalist$mosquito_collection) %in%
      drops)])
    ...
    description <- list(animais_summary = animais_summary, casa_table = casa_table, casa_summary = casa_summary,
      preco_summary = preco_summary, morte_table = morte_table, mosquiteiro_summary = mosquiteiro_summary, collection_table = collection_table,
      collection_summary = collection_summary, pessao_table = pessao_table, pessao_summary = pessao_summary, sporozoites_table = sporozoites_table)
    return(description)
  }
  ...
  analysis_of_proportions <- function(dissections) {
    ...
    M <- parous/dissected
    M_interval <- prop.test(x = parous, n = dissected, conf.level = 0.95, correct = FALSE)
    M_sample <- rbinom(1000, dissected, M)/dissected
    sac <- sum(dissections$Sac)
    A0 <- sac/parous
    A0_sample <- rbinom(1000, parous, A0)/parous
    A0_interval <- prop.test(x = sac, n = parous, conf.level = 0.95, correct = FALSE)
    fdenom <- 6379 + 791 + 5121
    fvalue <- 6379/fdenom
    f_sample <- rbinom(1000, fdenom, fvalue)/fdenom
    proportions <- list(M = M, M_interval = M_interval, M_sample = M_sample, A0 = A0, A0_sample = A0_sample, A0_interval = A0_interval,
  ...
  get_timeseries <- function(datalist) {
    ...
    alldates <- data.frame(date = as.numeric(as.Date(datalist$mosquito_collection$date, "%d/%m/%Y"))) - as.numeric(as.Date("26/06/2001",
      format = "%d/%m/%Y"))
  ...
  ################ Data descriptions
  Furvela_data <- list(animais = animais, casa = casa, morte = morte, mosquiteiro = mosquiteiro, mosquito_collection = mosquito_collection,
    pessao = pessao, sporozoites = sporozoites, dissections = dissections)
...
selectData <- function(firstquarter = 1, lastquarter = 31, model = "default", A0 = 0.6037, P = 0.4812, workfile = Furvela_data$workfile) {
  ...
  df1 <- data.frame(df %>%
    group_by(date) %>%
    dplyr::summarize(traps = n(), sum.Af.unfed1 = sum(Af.unfed1, na.rm = TRUE), mean.Af.unfed1 = mean(Af.unfed1,
      na.rm = TRUE), mean.Af.maleEx = mean(Af.maleEx, na.rm = TRUE), sum.Af.maleEx = sum(Af.maleEx, na.rm = TRUE),
      mean.Af.gravid = mean(Af.gravid, na.rm = TRUE), sum.Af.gravid = sum(Af.gravid, na.rm = TRUE), mean.Af.old = mean(Af.old,
        na.rm = TRUE), mean_temp = mean(avg_temp)))
  ...
  # df1b - all data df1c - all sampled dates df1d - all sampled dates with complete data for lags
  ...
  jagsdata <- list(sampleddates = i20, unfed1 = df1c$sum.Af.unfed1[1:i20], traps = df1c$traps[1:i20], days_with_complete_data = length(df1d$ptr),
    gravid = df1c$sum.Af.gravid[1:i20], ptr = df1d$ptr, lag1 = df1d$lag1, lag2 = df1d$lag2, lag3 = df1d$lag3, lag4 = df1d$lag4)
  ...
  if (model == "temperature") {
    jagsdata$mean_temp <- df1c$mean_temp
  }
...
summarise_by_method <- function(workfile = Furvela_data$workfile) {
  # houses observed
  ...
  df2 <- data.frame(workfile %>%
    group_by(Collection) %>%
    dplyr::summarize(mu.Af.unfed = mean(l_Af.unfed, na.rm = TRUE), mu.Af.part = mean(l_Af.part, na.rm = TRUE), mu.Af.fed = mean(l_Af.fed,
      na.rm = TRUE), mu.Af.semi = mean(l_Af.semi, na.rm = TRUE), mu.Af.gravid = mean(l_Af.gravid, na.rm = TRUE),
      mu.Af.male = mean(l_Af.male, na.rm = TRUE), Af.unfed = sum(Af.unfed, na.rm = TRUE), Af.part = sum(Af.part,
        na.rm = TRUE), Af.fed = sum(Af.fed, na.rm = TRUE), Af.semi = sum(Af.semi, na.rm = TRUE), Af.gravid = sum(Af_gravid,
        na.rm = TRUE), Af.male = sum(Af.male, na.rm = TRUE)))
...
summarise_by_quarter <- function(workfile = Furvela_data$workfile) {
  threemonths <- 365.25/4
  ...
  Af.femaleLT <- with(workfile, ifelse(Collection == "Light", log(Af.unfed + Af.part + Af.fed + Af.semi + Af_gravid +
    1), NA))
  df <- data.frame(cbind(Af.unfed1, Af.maleEx, Af.femaleLT, Af.gravid, quarter))
  df2 <- df %>%
    group_by(quarter) %>%
    dplyr::summarize(mu.Af.unfed1 = mean(Af.unfed1, na.rm = TRUE), mu.Af.maleEx = mean(Af.maleEx, na.rm = TRUE),
      mu.Af.femaleLT = mean(Af.femaleLT, na.rm = TRUE), mu.Af.gravid = mean(Af.gravid, na.rm = TRUE))
  df2 <- df2[!is.na(df2$quarter), ]
  library(data.table)
  df3 <- reshape2::melt(df2, id.vars = "quarter")
  df3$value <- exp(df3$value) - 1
  ticklabels <- c(rep("", 3), "2002", rep("", 3), "2003", rep("", 3), "2004", rep("", 3), "2005", rep("", 3), "2006",
    rep("", 3), "2007", rep("", 3), "2008", rep("", 3))
  plt <- ggplot(data = df3, aes(x = as.factor(quarter), group = variable)) + theme_bw() + theme(text = element_text(size = 12)) +
    scale_colour_manual(values = get_palette(), name = "Mosquito category", labels = c("An. funestus unfed or part fed",
      "An. funestus males (light trap)", "An. funestus females (light trap)", "An. funestus fed or gravid")) +
    geom_line(aes(y = value, colour = variable), linewidth = 2) + geom_vline(xintercept = 22.5) + scale_y_log10(name = "Williams mean (per trap night)") +
