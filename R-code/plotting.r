...
errbarPlot <- function(data, low, med, high, xname, yname, ylimits, logscale = TRUE, requireLegend = FALSE) {
  ...
  data$med <- med
  data$high <- high
  plt <- ggplot(data = data, aes(x = lag, group = yname)) + theme_bw() + theme(text = element_text(size = 12)) + scale_colour_manual(values = get_palette(),
    name = "Mosquito category") + geom_errorbar(aes(ymin = low, ymax = high, width = 0.5, colour = yname), show.legend = requireLegend) +
    theme(legend.position = c(0.75, 0.82)) + scale_fill_manual(values = get_palette(), guide = "none") + geom_point(aes(y = rs,
    colour = yname), size = 5, show.legend = FALSE) + scale_x_continuous(name = xname, limits = c(-1, 11), breaks = c(0,
    2, 4, 6, 8, 10))
  if (logscale)
    plt <- plt + scale_y_log10(name = yname, limits = ylimits)
  if (!logscale)
    plt <- plt + scale_y_continuous(name = "Spearman r", limits = ylimits)
  return(plt)
}
...
plotEstimatesByTime <- function(param, paramLabel, results = results, requirelegend = FALSE) {
  ...
  plt <- ggplot(data = toPlot, aes(x = (firstquarter + 1), group = jagsModel)) + theme_bw() + theme(text = element_text(size = 10)) +
    scale_colour_manual(values = get_palette(), name = "Model") + geom_errorbar(aes(ymin = X2.5., ymax = X97.5.,
    width = 0.5, colour = jagsModel), show.legend = FALSE) + scale_fill_manual(values = get_palette(), guide = "none") +
    geom_point(aes(y = X50., colour = jagsModel), shape = 21, size = 3, stroke = 1.5, show.legend = requirelegend) +
    theme(legend.position = c(0.6, 0.75)) + theme(legend.background = element_rect(colour = "black", fill = "white",
    linetype = "solid")) + scale_y_continuous(name = paramLabel) + scale_x_continuous(name = "Year", limits = c(8,
    32), breaks = c(8.5, 12.5, 16.5, 20.5, 24.5, 28.5), labels = ticklabels) + geom_vline(xintercept = 22.5)
  return(plt)
}
...
plotEstimatesByTemp <- function(param, paramLabel, results = results, requirelegend = FALSE) {
  ...
  plt <- ggplot(data = toPlot, aes(x = Index, group = jagsModel)) + theme_bw() + theme(text = element_text(size = 10)) +
    scale_colour_manual(values = get_palette(), name = NULL) + geom_errorbar(aes(ymin = X2.5., ymax = X97.5., width = 0.2,
    colour = jagsModel), show.legend = FALSE) + scale_fill_manual(values = get_palette()) + geom_point(aes(y = X50.,
    colour = jagsModel), shape = 21, size = 3, stroke = 1.5, show.legend = requirelegend) + theme(legend.position = c(0.35,
    0.09)) + guides(colour = guide_legend(nrow = 1, label.position = "left")) + theme(legend.background = element_rect(colour = "black",
    fill = "white", linetype = "solid"), legend.title = element_blank(), legend.key.height = unit(0.1, "cm")) +
    scale_y_continuous(name = paramLabel) + scale_x_continuous(name = "Weekly Mean Temperature", limits = c(0.5,
    5.5), breaks = c(1, 2, 3, 4, 5), labels = ticklabels)
  return(plt)
}
...
plotSimulations_vs_Inputs <- function(data, textLabel, xlim, nmodels = 5, requirelegend = FALSE) {
  plt <- ggplot(data = data, aes(x = input, y = X50., group = jagsModel)) + theme_bw(base_size = 9) + theme(legend.background = element_rect(colour = "black",
    fill = "white", linetype = "solid")) + theme(legend.position = c(0.8, 0.5)) + geom_point(aes(y = X50., colour = jagsModel,
    shape = jagsModel), size = 2, stroke = 1.5, show.legend = requirelegend) + scale_colour_manual(name = "Constraints",
    values = get_palette()) + scale_shape_manual(name = "Constraints", values = (seq(1:nmodels) - 1)) + geom_line(aes(x = input,
    y = input)) + scale_x_continuous(name = paste("Input", textLabel), limits = xlim) + scale_y_continuous(name = paste("Modelled",
    textLabel), limits = xlim)
  return(plt)
}
...
plotConsistency <- function(data, textLabel, nmodels = 5, requirelegend = FALSE) {
  ...
  plt <- ggplot(data = data, aes(x = days_with_complete_data, y = ratio, group = jagsModel)) + theme_bw(base_size = 9) +
    theme(legend.background = element_rect(colour = "black", fill = "white", linetype = "solid")) + theme(legend.position = c(0.1,
    0.32)) + geom_smooth(aes(y = sqe, colour = jagsModel), size = 1, show.legend = requirelegend) + scale_colour_manual(name = element_blank(),
    values = get_palette()) + scale_shape_manual(name = element_blank(), values = (seq(1:nmodels) - 1)) + scale_x_continuous(name = "Days with complete data") +
    scale_y_log10(name = paste("SqErr:", textLabel))
  return(plt)
}

# Functions for analysis and plotting of correlations ANALYSIS OF CORRELATIONS
plotCorrelations <- function() {
  ...
  plt1 <- with(corr, errbarPlot(data = corr, low = lower, med = rs, high = upper, xname = "lag (days)", yname = yname,
    ylimits = c(-0.3, 0.7), logscale = FALSE))
  autocorr <- calculate_autocorrelations(df1 = df1)
  levels(autocorr$yname) <- list(Male = "male", Gravid = "gravid", `Unfed or part fed` = "unfed1")
  plt2 <- with(autocorr, errbarPlot(data = autocorr, low = lower, med = rs, high = upper, xname = "lag (days)", yname = yname,
    ylimits = c(0.2, 0.7), logscale = FALSE, requireLegend = TRUE))
  library(cowplot)
...
plotSimulatedData <- function() {
  ...
  ggplot(data = toPlot, aes(x = time, y = value, colour = var)) + geom_point() + theme_bw() + theme(text = element_text(size = 12))
  return()
}
...
createTables_Plots <- function(input, plottype = "bias") {
  ...
  calculateBias <- function(df, Var) {
    df1 <- df %>%
      group_by(jagsModel) %>%
      summarise(N = n(), mean.ci = list(mean_ci((X50. - input)/input))) %>%
      unnest_wider(mean.ci)
    df1$Var <- Var
    return(df1)
  }

  resultsFemales <- simresults$parameters[which(simresults$parameters$jagsModel %in% c("exittrap1", "P_075", "A_03",
    "P_est", "Teu_known")), ]
  nmodels <- 5
  ...
  toPlot1a <- data.frame(resultsFemales[resultsFemales$Var == "Teu", ], input = rep(inputs_to_simulations$Teu, each = nmodels),
    days_with_complete_data = rep(inputs_to_simulations$days_with_complete_data, each = nmodels))

  toPlot1b <- data.frame(resultsFemales[resultsFemales$Var == "P", ], input = rep(inputs_to_simulations$P, each = nmodels),
    days_with_complete_data = rep(inputs_to_simulations$days_with_complete_data, each = nmodels))
  toPlot1c <- data.frame(resultsFemales[resultsFemales$Var == "resting", ], input = rep(inputs_to_simulations$resting,
    each = nmodels), days_with_complete_data = rep(inputs_to_simulations$days_with_complete_data, each = nmodels))

  # Plots of simulation results for males
  resultsMales <- simresults$parameters[which(simresults$parameters$jagsModel %in% c("Teu_known", "Tem_known")), ]
  nmodels <- 2
  ...
  library(cowplot)
  if (plottype == "bias") {
    p1a <- plotSimulations_vs_Inputs(data = toPlot1a, textLabel = "trapping efficiency: gravids", xlim = c(0, 8),
      requirelegend = TRUE)
    ...
    p2a <- plotSimulations_vs_Inputs(data = toPlot2a, textLabel = "daily survival: males", nmodels = 2, xlim = c(0,
      1))
    p2b <- plotSimulations_vs_Inputs(data = toPlot2b, textLabel = "trapping efficiency: males", nmodels = 2, xlim = c(1,
      10), requirelegend = TRUE)
